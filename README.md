...
subagent worker 是独立 pi 子进程，子进程的内存 state 是空的。如果 tool 依赖内存 state 就会失败。改成每次直接读 `tasks.jsonl` → 修改 → 写回。

写盘采用 **atomic rename**（`writeFile(tmp)` → `rename(tmp, target)`）：POSIX `rename` 在同一文件系统内是原子的，并发读者（`turn_end` 刷 widget、兄弟 worker）绝不会读到半写文件。

> **注意**：atomic rename 解决"读半写"，**不解决两个 worker 同时 read-modify-write 时的丢更新**。个人单机场景下窗口极窄（毫秒级）、后果仅是 status 偶发不一致（下次 turn_end 重读会刷新），按"出现可观察问题再上 `proper-lockfile`"处理。
...
`.exec-pending.json` 内带 `createdAt`，读取时若 **> 5min 视为过期**直接丢弃；`launchExecSession` 取消 / 抛错时也会主动 `clearExecPending`，避免下次任意 `session_start` 被残留文件劫持。
...
**多个 blocked 任务**会在同一次 agent_end 中**循环处理**（菜单标题会显示"还有 N 个阻塞任务"），所有决策完成后一次性把跳过数 + 重试说明合并发送给 executor，避免 executor 在两个 blocked task 间空跑 LLM 轮次。
...
| 文件 | 用途 |
|------|------|
| `.plans/<name>/tasks.jsonl` | 手动编辑可修复 task 状态（atomic rename 写入） |
| `.plans/plans.jsonl` | 注册表，删除对应行即"删除计划" |
| `.plans/.exec-pending.json` | 跨 session 握手文件，正常自动清除；> 5min 自动判过期 |
| pi session entries 中的 `plan-execute-state` | 同 session 内状态持久化；写入有幂等守卫，状态未变不重复 append |
